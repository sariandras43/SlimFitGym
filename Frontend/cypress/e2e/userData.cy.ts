describe('User data ', () => {
  ...
  describe('Edit User Data', () => {
    ...
    it('should persist updated name after submission', () => {
      cy.intercept('PUT', 'http://localhost:5278/api/auth/modify/3', { statusCode: 200 }).as('updateUser');
      cy.get('#name').clear().type('Jane Doe');
      cy.get('.edit').click();
      cy.wait('@updateUser').its('request.body').should('deep.include', { name: 'Jane Doe' });
      cy.get('#name').should('have.value', 'Jane Doe');
    });
  
    it('should show error for invalid email format', () => {
      cy.get('#email').clear().type('invalid-email');
      cy.get('.error-message').should('contain', 'Érvénytelen email cím');
    });
  
    it('should validate phone number format', () => {
      cy.get('#phone').clear().type('123');
      cy.get('.error-message').should('contain', 'Érvénytelen telefonszám');
    });
  });
